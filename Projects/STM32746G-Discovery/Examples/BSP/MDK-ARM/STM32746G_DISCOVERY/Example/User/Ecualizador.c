...
void eqParametros() {
    ...
    if (Vo[0] >= 1) c[0] = (1.0f - tanf(PI * fc[0] / SR)) / (1 + tanf(PI * fc[0] / SR));
    else c[0] = (Vo[0] - tanf(PI * fc[0] / SR)) / (Vo[0] + tanf(PI * fc[0] / SR));

    //parametros peak
    for (i = 1; i < N - 1; i++) {
        d[i] = -cos(2.0f * PI * fc[i] / SR);

        if (Vo[i] >= 1) c[i] = (1.0f - tanf(PI * fb[i] / SR)) / (1.0f + tanf(PI * fb[i] / SR));
        else c[i] = (Vo[i] - tanf(PI * fb[i] / SR)) / (Vo[i] + tanf(PI * fb[i] / SR));
    }

    //parametros high shelve
    if (Vo[N - 1] >= 1) c[N - 1] = (1.0f - tanf(PI * fc[N - 1] / SR)) / (1.0f + tanf(PI * fc[N - 1] / SR));
    else c[N - 1] = (1.0f - Vo[N - 1] * tanf(PI * fc[N - 1] / SR)) / (1.0f + Vo[N - 1] * tanf(PI * fc[N - 1] / SR));
}
...
int32_t eqShelve(int32_t in, int32_t i) {
    ...
    if (i == 0) in = (float32_t)0.5f * (Vo[i] - 1) * (in - y_0[i]) + in;
    else in = (float32_t)0.5f * (Vo[i] - 1) * (in + y_0[i]) + in;
