...
float32_t autowahLFO(int32_t modulacion)
{
	...
	case ENVOLVENTE:
		...
		if(flag == 0)
			fcentral = fcentral + 2.0f * deltafenv;
		if(flag == 1)
			fcentral = fcentral - 2.0f * deltafenv;
		if(fcentral >= ffinal)
			flag = 1;
...
void autowahMod (GUIElement *e)
{
	...
	if (db->value <= 0.2f)
		modulacion = CUADRADA;
	else if(db->value > 0.2f && db->value <= 0.4f)
		modulacion = TRIANGULAR;
	else if(db->value > 0.4f && db->value <= 0.6f)
		modulacion = RAMPA_ASC;
	else if(db->value > 0.6f && db->value <= 0.8f)
		modulacion = RAMPA_DESC;
	else if(db->value > 0.8f && db->value < 1.0f)
