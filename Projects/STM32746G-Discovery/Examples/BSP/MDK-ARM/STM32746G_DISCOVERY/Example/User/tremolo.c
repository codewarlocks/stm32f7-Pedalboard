...
float32_t tremoloLFO() {
    switch (modulacion) {
        ...
        case TRIANGULAR:
            ...
            if (flag == 0) coef = coef + 2.0f * delta;

            if (flag == 1) coef = coef - 2.0f * delta;
...
void tremoloMod(GUIElement *e) {
    ...
    if (db->value <= (float32_t)0.33f) modulacion = SINUSOIDAL;
    else if (db->value > (float32_t)0.33f && db->value <= (float32_t)0.66f) modulacion = CUADRADA;
    else if (db->value > (float32_t)0.66f && db->value < 1.0f) modulacion = TRIANGULAR;
    else if (db->value == 1) modulacion = RAMPA_ASC;
}
