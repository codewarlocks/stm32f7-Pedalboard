...
int32_t vibratoEfecto (int32_t entrada)
{
	...
	if (flag == 1)
	{
		...
		frac = demora - d;
		if (cont-d-1 >= 0)
			salida = frac * line[cont-d-1] + (1.0f-frac) * line[cont-d];
		if (cont-d == 0)
			salida = frac * line[VIBRATO_SIZE-1] + (1.0f-frac) * line[cont-d];
		if (cont-d < 0)
			salida = frac * line[VIBRATO_SIZE-d+cont-1] + (1.0f-frac) * line[VIBRATO_SIZE-d+cont];
	}
	cont++;
	return salida;
}
